...
# Stripe calls this directly, not through API Gateway - same reasoning as
# stripe-webhook: it can't sign requests with AWS SigV4, and it isn't a
...
# registered as this order-payment webhook's destination in the Stripe
# Dashboard.
#
...
# Stripe-Signature header against ORDER_STRIPE_WEBHOOK_SECRET and rejects
