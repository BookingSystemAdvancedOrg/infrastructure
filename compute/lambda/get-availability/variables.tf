...
variable "role_arn" {
  description = "ARN of the GetAvailabilityFn execution role (security/iam/get-availability)"
  type        = string
  sensitive   = true
}

variable "ecr_repository_url" {
  description = "Repository URL of the GetAvailabilityFn ECR repository (storage/ecr/get-availability) — the repo itself is owned there, not created in this module"
  type        = string
  sensitive   = true
}
