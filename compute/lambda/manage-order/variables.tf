...
variable "role_arn" {
  description = "ARN of the ManageOrderFn execution role (security/iam/manage-order)"
  type        = string
  sensitive   = true
}

variable "ecr_repository_url" {
  description = "Repository URL of the ManageOrderFn ECR repository (storage/ecr/manage-order) — the repo itself is owned there, not created in this module"
  type        = string
  sensitive   = true
}
